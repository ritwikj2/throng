...
try {
  if (config.provider === "local")
    throw new Error("Select an external provider and model in Connect brain first.");
  if (config.configurationError) throw new Error(config.configurationError);
  const world = createWorld(20260914, "Synthetic brain connection check");
  world.cognitionMode = "model";
  applyCommand(world, { type: "hatch" });
  stepWorld(world, 0.1);
  cognition = createCognition(config);
  console.log("Checking one decision from the selected provider in an isolated synthetic world.");
  ...
  console.error(`Brain check failed: ${error.message}`);
  process.exitCode = 1;
} finally {
  cognition?.stop();
}
